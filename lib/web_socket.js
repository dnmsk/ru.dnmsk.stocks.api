...
export default class WS {
  ...
  __processMsg(msg) {
    ...
    if (subscriber) {
      ...
      if (msg.a) {
        return subscriber.acts(msg);
      }
      return subscriber.onReceive(msg.d);
    }
    this.__sendToWs(
      ...
      this.webSocket.addEventListener('message', data => {
        try {
          if (data.data instanceof Blob) {
            ...
            this.__processMsg(JSON.parse(data.data));
          }
        } catch(ex) {
          console.log(ex);
        }
  ...
  connectChannel(channelName, data, onReceive, onConnect, acts) {
    const channelNameUpperCase = channelName.toUpperCase();
    if (!this.subscribers[channelNameUpperCase]) {
      const channel = new WsChannel(this, channelName, onReceive, onConnect, acts);
