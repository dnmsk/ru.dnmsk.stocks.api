...
function onReadyState(ws, force) {
  return new Promise((success, reject) => {
    function loop() {
      ...
      switch(v) {
        case 0:
          return setTimeout(loop, 5);
        case 1:
          return success(ws.webSocket);
        default:
          return reject(ws.webSocket);
      }
    }
    loop();
  });
...
export default class WS {
  ...
  __processMsg(msg) {
    ...
    if (msg.c) {
      ...
      if (subscriber) {
        ...
        if (msg.d) {
          if (msg.d.connected === true) {
            return subscriber._onConnect(msg.d);
          }

          if (msg.d.disconnected === true) {
            delete this.subscribers[subscriber.channelName.toUpperCase()];
            if (subscriber.channelID) {
              delete this.subscribersById[subscriber.channelID];
              subscriber.channelID = undefined;
            }
            return subscriber._onDisconnect(msg.d);
          }
        }
      }
    } else {
  ...
  connectChannel({channelName, onReceive, onConnect = (() => {}), onDisconnect = (() => {}), acts}) {
    ...
    if (!channel) {
      return new Promise((success, fail) => {
        channel = new WsChannel({
          ...
          onReceive,
          onConnect: data => {
            const obj = { channel, data };
            success(obj);
            onConnect(obj);
            channel.onConnect = d => onConnect({channel, data: d});
          },
          onDisconnect,
          acts
        });
  ...
  _leaveChannel(subscriber) {
    const subscriberInList = this.subscribers[subscriber.channelName.toUpperCase()];
    if (!subscriberInList) {
